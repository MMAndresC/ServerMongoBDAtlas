...
const getAffiliations = (criteria) => {
    const promise = new Promise((resolve, reject) => {
        const idFound = Affiliation.findOne({faction: criteria},{faction:false, description:false});
        if(!idFound){
            reject('No encontrado');
        }else{
            console.log('esto?',idFound);
            resolve(idFound);
        }
      })
...
const getIdFaction = async(criteria) =>{
    try{
        const idFaction = await getAffiliations(criteria);
        if(idFaction) {
            console.log('Success retrieving id faction');
            return idFaction;
        }    
    }catch(err){
        console.log('Fail retrieving id faction');
    }
}

 const ow = getIdFaction('Overwatch');  
 console.log('ow',ow);
 const talon = getIdFaction('Talon')
